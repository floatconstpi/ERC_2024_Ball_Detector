            ...
            for i in range(1, num_labels):
                ...
                if area > 150:  
                    ...
                    if math.isfinite(point_cloud_value[2]):
                        ...
                        angle = math.atan(point_cloud_value[0]/point_cloud_value[1])
                        i = 0;
                        if (i % 1000 == 0):
                            print("M1",arduinoMap(angle * 1000, 200, 1600, 1300, 2000) % 10 if angle >= 0.2 else None)
                            print("M2",arduinoMap(-angle * 1000, 200, 1600, 1300, 2000) if angle <= -0.2 else None)
                        i+=1
                        cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 255, 0), 2)
                    if center_x < 640:
                        cv2.putText(frame, f"LEFT D: {distance:.2f} mm, O: {point_cloud_value[0]:.2f}, A: {angle:.2f}", (x, y - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
                    else:
                        cv2.putText(frame, f" RIGHT D: {distance:.2f} mm, O: {point_cloud_value[0]:.2f}, A: {angle:.2f}", (x, y - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
